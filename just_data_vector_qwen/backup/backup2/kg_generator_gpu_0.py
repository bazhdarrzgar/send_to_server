...
class KMCKnowledgeGraph:
    ...
    def store_triple(self, triple: Dict):
        try:
            ...
            if rel_clean[0].isdigit():
                rel_clean = "R_" + rel_clean
            ...
            logger.error(f"[GPU0] Failed to store triple in Neo4j: {triple}. Error: {e}")
    ...
    def process_dataset(self):
        ...
        logger.info(f"[GPU0] {total} entries total | {already} already processed | {total - already} remaining.")

        neo4j_ok = True  # Track Neo4j availability

        for entry in tqdm(data, desc="GPU0"):
            ...
            if triples:
                # ── Persist to JSONL (always) ──
                try:
                    with open(OUTPUT_FILE, 'a', encoding='utf-8') as f_out:
                        record = {"id": entry_id, "triples": triples}
                        f_out.write(json.dumps(record, ensure_ascii=False) + "\n")
                        f_out.flush()
                    logger.info(f"[GPU0] ID={entry_id}: Wrote {len(triples)} triples to {OUTPUT_FILE}.")
                except Exception as e:
                ...
                if neo4j_ok:
                    for triple in triples:
                        try:
                            ...
                            neo4j_ok = False  # Stop spamming the log
