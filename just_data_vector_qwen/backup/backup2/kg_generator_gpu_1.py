...
class KMCKnowledgeGraph:
    ...
    def store_triple(self, triple: Dict):
        try:
            ...
            logger.error(f"[GPU1] Failed to store triple in Neo4j: {triple}. Error: {e}")
    ...
    def process_dataset(self):
        ...
        logger.info(f"[GPU1] {total} entries total | {already} already processed | {total - already} remaining.")

        neo4j_ok = True  # Track Neo4j availability

        for entry in tqdm(data, desc="GPU1"):
            ...
            if triples:
                ...
                try:
                    with open(OUTPUT_FILE, 'a', encoding='utf-8') as f_out:
                        ...
                        f_out.flush()
                    logger.info(f"[GPU1] ID={entry_id}: Wrote {len(triples)} triples to {OUTPUT_FILE}.")
                except Exception as e:
