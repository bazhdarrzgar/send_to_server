#!/usr/bin/env python3
"""
generate_dataset_first.py (llama-cpp-python version) - Fourth Dataset Instance

For every .txt file under Extra/Data_Four:
  - Standalone files  (e.g. 1-10.txt):   run the prompt 11 times → 11 JSON files
                                           → merge all 11 into one final JSON file
  - Part files        (e.g. 163267_part1.txt, 163267_part2.txt, …):
                        run the prompt 11 times per part → 11 JSON files per part
...
DATA_DIR   = "UKH-AIIC-KA_Just_TXT_file/part3"
OUTPUT_DIR = "UKH-AIIC-KA_Just_TXT_file_Output/part3" 
MODEL_NAME = "gemma-3-27b-it"
NUM_RUNS   = 1

# llama-cpp-python server address (OpenAI compatible)
# Note: Using Port 8013 for the Fourth Dataset instance
# Use a custom httpx client with proxy disabled to avoid system proxy interference
client = OpenAI(
...
def call_model(prompt: str) -> str:
    """Send a prompt to llama-cpp-python (OpenAI API) and return the raw response string.
...
def process_single_txt(txt_path: str, output_dir: str, base_name: str):
    ...
    for run in range(1, NUM_RUNS + 1):
        ...
        # Small pause to allow the llama.cpp server to breathe
        time.sleep(2)
