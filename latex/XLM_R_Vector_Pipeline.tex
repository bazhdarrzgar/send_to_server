...
\title{XLM-R Vector Generation and Visualization Toolkit}
...
\midrule
Vector Generator & \hyperref[sec:generator]{Section \ref*{sec:generator}} & Generates vector embeddings for a batch of texts using the XLM-R large model. \\
...
\midrule
Setup Script & \hyperref[sec:setup]{Section \ref*{sec:setup}} & Prepares the environment by installing dependencies and caching the XLM-R model. \\
Model Loader & \hyperref[sec:loader]{Section \ref*{sec:loader}} & A dedicated script to trigger the Hugging Face download of the XLM-R model for caching. \\
...
\textbf{Purpose:} We use this script to generate vector embeddings for the Kurdish medical dataset using the \texttt{xlm-roberta-large} model. It includes batching, checkpointing, and GPU acceleration.
...
\textbf{Purpose:} We use this shell script to prepare the environment, install required Python dependencies from \texttt{requirements.txt}, create a \texttt{.env} template, and cache the XLM-R model.
...
\textbf{Purpose:} A dedicated script used to download and cache the \texttt{xlm-roberta-large} model from Hugging Face into the local environment before running the main pipeline.
